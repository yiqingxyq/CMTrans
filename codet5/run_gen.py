...
class GumbelNoiseLogitsProcessor(LogitsProcessor):
    r"""
    ...
    def __call__(self, input_ids: torch.Tensor, scores: torch.Tensor) -> torch.FloatTensor:
        if self.temperature != 1.0:
            processed_scores = nn.functional.log_softmax(
                    scores / self.temperature, dim=-1
            )
        else:
            processed_scores = scores
        scale = torch.mean(torch.std(processed_scores, dim=1)) / 50
        processed_scores = Gumbel(processed_scores, scale=scale).sample()
...
def eval_bleu_epoch(args, eval_data, eval_examples, model, tokenizer, split_tag, criteria):
    ...
    if args.use_gumbel_noise:
        processors.append(GumbelNoiseLogitsProcessor(args.temp))
        
    for bid,batch in enumerate(tqdm(eval_dataloader, total=len(eval_dataloader), desc="Eval bleu for {} set".format(split_tag))):  
        ...
        with torch.no_grad():
            if args.model_type == 'roberta':
                ...
                # preds: [bs * num_seq, seq_len]
                bs = source_ids.shape[0]
                top_preds = list(preds.reshape(bs, args.num_return_sequences, -1).cpu().numpy())
            pred_ids.extend(top_preds)

    print('End of generation')
    pred_nls = [[tokenizer.decode(id[sid], skip_special_tokens=True, clean_up_tokenization_spaces=False) for sid in range(args.num_return_sequences)] for id in pred_ids]
    ...
    if args.task in ['defect']:
        ...
        dev_accs = {}
        bleus = {}
        with open(gold_fn, 'w', encoding='utf8') as f1, \
        ...
        if args.num_return_sequences == 1:     
            ...
            with open(output_fn , 'w', encoding='utf8') as f:
                for pred_nl, gold in zip(pred_nls, eval_examples):
                    dev_accs[0].append(pred_nl[0].strip() == gold.target.strip())
                    f.write(pred_nl[0].strip() + '\n')
            bleus[0] = round(_bleu(gold_fn, output_fn), 2)
        else:
            for candidate_id in range(args.num_return_sequences):
                dev_accs[candidate_id] = []
                with open(output_fn + str(candidate_id), 'w', encoding='utf8') as f:
                    for pred_nl, gold in zip(pred_nls, eval_examples):
                        dev_accs[candidate_id].append(pred_nl[candidate_id].strip() == gold.target.strip())
                        f.write(pred_nl[candidate_id].strip() + '\n')
                bleus[candidate_id] = round(_bleu(gold_fn, output_fn + str(candidate_id)), 2)
        avg_em = np.mean([dev_accs[i] for i in range(args.num_return_sequences) ]) * 100
        max_em = np.mean([np.max([dev_accs[i][j] for i in range(args.num_return_sequences)]) for j in range(len(pred_nls)) ] ) * 100
        avg_bleu = np.mean([bleus[x] for x in bleus])
        max_bleu = np.max([bleus[x] for x in bleus])
        result = {'em': avg_em, 'bleu': avg_bleu, 'max_em': max_em, 'max_bleu': max_bleu}
    else:
...
def main():
    ...
    if args.do_train:
        if args.local_rank in [-1, 0] and args.data_num == -1:
            summary_fn = args.output_dir + '/tensorboard'

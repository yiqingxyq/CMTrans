...
def get_codebleu(
    ...
    assert len(hypothesis) == len(references), f"hypothesis: {len(hypothesis)}, references: {len(references)}"
    ...
    # NOTE: commented as we are not calculating the ngram match:
    ...
    print(
        'Ngram match:\t%.2f\nSyntax match:\t%.2f\nDataflow match:\t%.2f' %
        (ngram_match_score*100, syntax_match_score*100, dataflow_match_score*100)
    )
    ...
    code_bleu_score = (alpha * ngram_match_score \
                      + gamma * syntax_match_score \
                      + theta * dataflow_match_score) / (alpha+gamma+theta)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lang', type=str, required=True,
                        choices=['java', 'js', 'c_sharp', 'cpp', 'php', 'go', 'python', 'ruby'],
                        help='programming language')
    parser.add_argument('--params', type=str, default='0.25,0.25,0.25,0.25', help='alpha, beta, gamma and theta')

    args = parser.parse_args()

    code_bleu_score = get_codebleu(args.ref, args.hyp, args.lang, args.params, args.txt_ref)
    print('\x1b[34;1mCodeBLEU score:\x1b[0m %.2f' % (code_bleu_score * 100.0))
